...
* [use-wallpaper-state.js](use-wallpaper-state.js) - the persistent wallpaper settings, stored in `localStorage` under the same key as the old prototype so a returning user's saved setup carries over. Phase 2 replaces cards-era fields (`base`, `destinations`, `mode`) with `home`, `travel`, and ladder settings.
